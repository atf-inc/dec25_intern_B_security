...
async def sync_emails(
    ...
    try:
        ...
        gmail_emails = await asyncio.wait_for(
            run_in_threadpool(fetch_gmail_messages, x_google_token, 20), timeout=30.0
        )
        ...
        if count > 0:
            ...
            if new_email_ids:
                ...
                for eid in new_email_ids:
                    await redis.xadd(EMAIL_INTENT_QUEUE, {'email_id': eid})
                logger.info(f'Queued {len(new_email_ids)} emails for analysis in stream')
